...
import org.ys.transaction.domain.inteface.CouponService;
...
public class CouponServiceImpl implements CouponService {
    ...
    public DomainResult createCoupon(Map<String, Object> params) {
        try {
            ...
            if (result > 0) {
                return DomainResult.success("优惠券创建成功");
            } else {
                return DomainResult.error("优惠券创建失败");
            }
        } catch (Exception e) {
    ...
    @Override
    public DomainResult claimCoupon(Long userId, Long couponId) {
        try {
            // 检查用户是否存在
            ...
            if (coupon == null) {
                return DomainResult.error("优惠券不存在");
            }

            // 检查优惠券状态
            if (coupon.getStatus() != 1) {
                return DomainResult.error("优惠券已失效");
            }

            // 检查优惠券是否在有效期内
            LocalDateTime now = LocalDateTime.now();
            if (coupon.getValidStartTime() != null && now.isBefore(coupon.getValidStartTime())) {
                return DomainResult.error("优惠券不在有效期内");
            }
            ...
            if (totalCount <= usedCount) {
                return DomainResult.error("优惠券已领完");
            }
            ...
            if (userCoupons != null && userCoupons.size() >= perUserLimit) {
                return DomainResult.error("您已达到领取上限");
            }
            ...
            if (result > 0) {
                ...
                return DomainResult.success("领取成功");
            } else {
                return DomainResult.error("领取失败");
            }
        } catch (Exception e) {
            log.error("领取优惠券失败: userId={}, couponId={}, error={}", userId, couponId, e.getMessage(), e);
            return DomainResult.error("领取失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getUserCoupons(Long userId, Integer status, Integer pageNum, Integer pageSize) {
        try {
            // 分页参数
            ...
            return DomainResult.error("获取优惠券失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getAvailableCoupons(Long userId, BigDecimal amount, String goodsIds) {
        try {
            // 查询用户可用的优惠券
            QueryWrapper<YsUserCoupon> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("user_id", userId);
            ...
            return DomainResult.success(availableCoupons);
        } catch (Exception e) {
    ...
    @Override
    public DomainResult useCoupon(Long userId, Long couponId, Long orderId) {
        try {
            // 查询用户优惠券
            QueryWrapper<YsUserCoupon> queryWrapper = new QueryWrapper<>();
            ...
            if (userCoupon == null) {
                return DomainResult.error("优惠券不存在或已使用");
            }
            ...
            return DomainResult.success("优惠券使用成功");
        } catch (Exception e) {
            log.error("使用优惠券失败: userId={}, couponId={}, error={}", userId, couponId, e.getMessage(), e);
            return DomainResult.error("使用失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getCouponDetail(Long couponId) {
        try {
            YsCoupon coupon = couponDao.selectById(couponId);
            if (coupon == null) {
                return DomainResult.error("优惠券不存在");
            ...
            return DomainResult.success(coupon);
        } catch (Exception e) {
            log.error("获取优惠券详情失败: couponId={}, error={}", couponId, e.getMessage(), e);
            return DomainResult.error("获取详情失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getCouponList(Map<String, Object> params) {
        try {
    ...
    public DomainResult updateCoupon(Map<String, Object> params) {
        try {
            ...
            if (result > 0) {
                return DomainResult.success("更新成功");
            } else {
                return DomainResult.error("更新失败");
            }
        } catch (Exception e) {
    ...
    @Override
    public DomainResult deleteCoupon(Long couponId) {
        try {
            int result = couponDao.deleteById(couponId);
            if (result > 0) {
                return DomainResult.success("删除成功");
            } else {
                return DomainResult.error("删除失败");
            }
        } catch (Exception e) {
