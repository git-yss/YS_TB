...
import org.ys.transaction.domain.inteface.OrderDetailService;
...
public class OrderDetailServiceImpl implements OrderDetailService {
    ...
    @Override
    public DomainResult getOrderDetail(Long orderId) {
        try {
            // 查询订单信息
            ...
            return DomainResult.success(orderDetail);
        } catch (Exception e) {
    ...
    @Override
    public DomainResult getUserOrders(Long userId, String status, Integer pageNum, Integer pageSize) {
        try {
            // 分页参数
    ...
    @Override
    public DomainResult applyRefund(Map<String, Object> params) {
        try {
            ...
            if (!order.getStatus().equals(String.valueOf(OrderStatusEnum.PAID.getCode())) &&
                ...
                return DomainResult.error("只有已支付或已完成的订单才能申请退款");
            }
            ...
            return DomainResult.success("退款申请已提交");
        } catch (Exception e) {
            log.error("申请退款失败: {}", e.getMessage(), e);
            return DomainResult.error("申请退款失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult cancelRefund(Long orderId) {
        try {
            // 查询订单信息
            List<YsOrder> orders = orderDao.selectsById(orderId);
            if (orders == null || orders.isEmpty()) {
            ...
            if (!order.getStatus().equals(String.valueOf(OrderStatusEnum.REFUNDING.getCode()))) {
                return DomainResult.error("只有退款中的订单才能取消退款");
            }
            ...
            return DomainResult.success("退款已取消");
        } catch (Exception e) {
            log.error("取消退款失败: orderId={}, error={}", orderId, e.getMessage(), e);
            return DomainResult.error("取消退款失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult confirmReceipt(Long orderId) {
        try {
            // 查询订单信息
            List<YsOrder> orders = orderDao.selectsById(orderId);
            if (orders == null || orders.isEmpty()) {
            ...
            if (!order.getStatus().equals(String.valueOf(OrderStatusEnum.SHIPPED.getCode()))) {
                return DomainResult.error("只有已发货的订单才能确认收货");
            }
    ...
    @Override
    public DomainResult cancelOrder(Long orderId, String reason) {
        try {
            // 查询订单信息
            List<YsOrder> orders = orderDao.selectsById(orderId);
            if (orders == null || orders.isEmpty()) {
            ...
            if (!order.getStatus().equals(String.valueOf(OrderStatusEnum.PENDING_PAYMENT.getCode()))) {
                return DomainResult.error("只有待支付的订单才能取消");
            }
    ...
    @Override
    public DomainResult getLogisticsInfo(Long orderId) {
        try {
            // 查询订单信息
            List<YsOrder> orders = orderDao.selectsById(orderId);
            if (orders == null || orders.isEmpty()) {
            ...
            if (!order.getStatus().equals(String.valueOf(OrderStatusEnum.SHIPPED.getCode())) &&
                !order.getStatus().equals(String.valueOf(OrderStatusEnum.FINISHED.getCode()))) {
                return DomainResult.error("订单尚未发货");
            }
...
//                }
            ...
            return DomainResult.success(logisticsInfo);
        } catch (Exception e) {
            log.error("查询物流信息失败: orderId={}, error={}", orderId, e.getMessage(), e);
            return DomainResult.error("查询物流信息失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult reorder(Long userId, Long orderId) {
        try {
            // 查询订单信息
            List<YsOrder> orders = orderDao.selectsById(orderId);
            if (orders == null || orders.isEmpty()) {
            ...
            return DomainResult.success("已加入购物车");
        } catch (Exception e) {
            log.error("重新购买失败: userId={}, orderId={}, error={}", userId, orderId, e.getMessage(), e);
            return DomainResult.error("重新购买失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult deleteOrder(Long userId, Long orderId) {
        try {
            // 查询订单信息
            List<YsOrder> orders = orderDao.selectsById(orderId);
            if (orders == null || orders.isEmpty()) {
            ...
            if (!order.getStatus().equals(String.valueOf(OrderStatusEnum.FINISHED.getCode())) &&
                ...
                return DomainResult.error("只能删除已完成或已取消的订单");
            }
            ...
            return DomainResult.success("订单已删除");
        } catch (Exception e) {
            log.error("删除订单失败: userId={}, orderId={}, error={}", userId, orderId, e.getMessage(), e);
            return DomainResult.error("删除订单失败: " + e.getMessage());
        }
    }
