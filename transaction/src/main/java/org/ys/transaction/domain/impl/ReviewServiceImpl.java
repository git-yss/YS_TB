...
import org.ys.transaction.domain.inteface.ReviewService;
...
public class ReviewServiceImpl implements ReviewService {
    ...
    @Override
    public DomainResult addReview(Map<String, Object> params) {
        try {
            ...
            if (rating < 1 || rating > 5) {
                return DomainResult.error("评分必须在1-5之间");
            }
            ...
            if (!order.getStatus().equals("4")) { // 假设4是已完成状态
                return DomainResult.error("只能对已完成的订单进行评价");
            }
            ...
            if (existingReview != null) {
                return DomainResult.error("您已经评价过该商品");
            }
            ...
            if (result > 0) {
                ...
                return DomainResult.success("评价成功");
            } else {
                return DomainResult.error("评价失败");
            }
        } catch (Exception e) {
            log.error("添加评价失败: {}", e.getMessage(), e);
            return DomainResult.error("添加评价失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getReviewsByGoodsId(Long goodsId, Integer pageNum, Integer pageSize) {
        try {
            // 分页参数
            ...
            return DomainResult.error("获取评价失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getReviewsByUserId(Long userId, Integer pageNum, Integer pageSize) {
        try {
            // 分页参数
            if (pageNum == null || pageNum < 1) {
                pageNum = 1;
    ...
    @Override
    public DomainResult replyReview(Long reviewId, String replyContent) {
        try {
            // 参数校验
            if (replyContent == null || replyContent.trim().isEmpty()) {
                return DomainResult.error("回复内容不能为空");
            }

            // 查询评价
            YsProductReview review = reviewDao.selectById(reviewId);
            if (review == null) {
                return DomainResult.error("评价不存在");
            }

            // 检查是否已经回复过
            if (review.getReplyContent() != null && !review.getReplyContent().isEmpty()) {
                return DomainResult.error("已经回复过该评价");
            }
            ...
            if (result > 0) {
                ...
                return DomainResult.success("回复成功");
            } else {
                return DomainResult.error("回复失败");
            }
        } catch (Exception e) {
            log.error("回复评价失败: reviewId={}, error={}", reviewId, e.getMessage(), e);
            return DomainResult.error("回复失败: " + e.getMessage());
        }
    }

    @Override
    public DomainResult getReviewStats(Long goodsId) {
        try {
